...
def listTrackCandidates(AUXHits, AUXExtrapolatedGlobalSSIDs, DFGlobalSSIDs):
    '''Returns all combinations of hit candidates for track fitting in the form
    [([track candidates for 8L input], track sector ID), ([track candidates], sector ID)...],
    where [track candidates] = [[16 hit coords], [16 hit coords]...], and layer 0 is the first hit coords.
    ...
    for trackInfo in allSortedCoordinates:
        ...
        for layer in range(12):
            ...
            for possibleHit in trackInfo[layer]:
                if type(possibleHit) is not list:
                    possibleHit = [possibleHit]
    ...
    return list(zip(trackCandidates, sectorIDs))
