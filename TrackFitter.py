...
def fitTracks(trackCandidates, TFConstants):
    ...
    for trackInfo in trackCandidates:
        ...
        '''
        # NOTE: Markus's indices are backwards from mine
        candidateSet = [[1969, 408, 2128, 1920, 1144, 1360, 88, 863, 933, 884, 76, 96, 776, 784, 25, 302]]

        TF_S = np.array([[-0.004433304071426, 0.013941973447800, 0.012529909610748, -0.010214924812317, -0.029214918613434, -0.103371143341064, 0.017020225524902, 0.090527296066284, -0.030548334121704, -0.012579888105393, 0.022223472595215],
                        ...
                        [-0.011970192193985, -0.005463331937790, 0.004391744732857, 0.638578414916992, 0.506456375122070, -0.110317945480347, 0.408686637878418, -0.048567771911621, -0.025058448314667, -0.072223901748657, -0.032361507415772]]).transpose()
        TF_h = np.array([-46.852661132812500, 310.626953125000000, 54.133178710937500, -39.045288085937500, -71.503173828125000, 90.035644531250000, -31.360839843750000, 416.453125000000000, 536.478515625000000, 138.770507812500000, -90.362548828125000]).transpose()
        TF_q = np.array([-11.147644042968800, -30.406311035156200, -0.676544189453125, 1.763809204101560, -0.000823061913252]).transpose()
        TF_c = np.array([[0.014632701873779, 0.000364933162928, 0.000004984016414, -0.000270121730864, -0.000000200557224],
                        ...
                        [0.000676380470395, 0.000504646450281, -0.000037204357795, 0.000004210363841, 0.000000424168320]]).transpose()
        '''
        ...
        for trackCandidate in candidateSet:
            trackParameters, chi2 = fitTrack(trackCandidate)
            if bestChi2 == -1 or chi2 < bestChi2:
                # print("Found better fit!")  # CHECKPOINT
                bestChi2 = chi2
                bestTrackParameters = trackParameters
                bestTrack = trackCandidate
        if bestChi2 > 40:  # not good enough - do recovery fits
            moreTrackCandidates = []
            for trackCandidate in candidateSet:
                for dropLayers in [[0, 1], [9], [11], [15]]:  # drop each of these separately (pixel layer 0, SCT layers 5, 7, 11)
        ...
        # print("Best track parameters were", [round(param, 3) for param in bestTrackParameters], "with a chi2 value of", bestChi2)
        bestTracks.append([bestTrack, bestTrackParameters, bestChi2])
